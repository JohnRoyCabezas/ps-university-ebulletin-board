  ...
  const uploadAvatar = (uploadedAvatar) => {
    if (
      ["jpg", "png", "jpeg"].includes(
        uploadedAvatar.name.substring(uploadedAvatar.name.lastIndexOf(".") + 1)
      )
    ) {
      const formData = new FormData();
      formData.append("avatar", uploadedAvatar);
      AvatarUploadApi.upload({ formData })
        .then((res) => {
          setParams({
            ...params,
            avatar: `http://localhost:8000/Avatars/${res.data}`,
          });
        })
        .catch(function () {
            ...
            <form className="mt-1">
              <div className="flex flex-col items-center justify-center mb-4">
                ...
                <div className="flex justify-center my-2 flex-col">
                  <label
                    htmlFor="dropzone"
                    className="bg-white border text-center rounded px-3 py-1 cursor-pointer hover:bg-gray-200"
                  >
                    Upload Photo
                  </label>
                  <input
                ...
                {(valid.format || valid.size) &&
                  (valid.format ? (
                    <span className="text-red-600 text-xs relative italic p-1">
                      <FontAwesomeIcon icon={faInfoCircle} /> Must be in
                      '.jpeg', '.jpg', and '.png' format
                    </span>
                  ) : (
                    <span className="text-red-600 text-xs relative italic p-1">
                      <FontAwesomeIcon icon={faInfoCircle} /> Avatar size must
                      be 2mb below
                    </span>
                  ))}
              </div>
